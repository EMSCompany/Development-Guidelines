...
```tsx
// ✅ Deviation from a SHOULD, justified at the point of deviation
// SHOULD-EXCEPTION(state-and-data): whole-store read is intentional here;
// this devtools panel renders every slice. Approved by @reviewer in PR #214.
const everything = useStore();
```

```tsx
// ❌ Silent deviation with no reason and no reviewer sign-off
const everything = useStore();
```
