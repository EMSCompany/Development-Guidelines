...
```tsx
// ✅ Native button: keyboard, focus, and role come for free
...
```tsx
// ❌ Reinvented button: no keyboard, no focus, role bolted on
...
```tsx
// ✅ Action stays in the DOM and focusable; hover only restyles it
<button className="opacity-60 focus-visible:opacity-100 group-hover:opacity-100">
  Delete
</button>
```

```tsx
// ❌ Action is mounted only on hover: invisible to keyboard and touch
...
```tsx
// ✅ Visible ring for keyboard focus
...
```tsx
// ❌ Focus removed with no replacement: keyboard users are lost
...
```tsx
// ✅ Error conveyed by icon and text, not color alone
<p className="text-destructive flex items-center gap-1">
  <AlertCircleIcon /> Card declined
</p>
```

```tsx
// ❌ Only color distinguishes the error
<p className="text-destructive">Card declined</p>
```
...
```tsx
// ✅ Label tied to control; error described and flagged
...
```tsx
// ❌ Placeholder as label; error not associated or announced
...
```tsx
// ❌ Screen reader announces nothing actionable
<button>
  <TrashIcon />
</button>
```
...
```tsx
// ✅ Live region announces async status without stealing focus
<p aria-live="polite">{isSaving ? "Saving..." : "Saved"}</p>
```

```tsx
// ❌ Redundant role on an element that already has it
